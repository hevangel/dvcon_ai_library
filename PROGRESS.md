...
- the `dvcon-submit` skill was exercised end to end against the **live** DVCon U.S. 2027 stage (`81951`, deadline Sep 7 2026) by writing this repo up as a DVCon extended abstract. `submissions/dvcon-us-2027/abstract.md` (1,021 body words, within the 600-1200 limit) converts through the bundled IEEE template to a 3-page US Letter PDF (2 body pages + a reference tail, Times New Roman embedded, unencrypted, no page numbers, 108 KB). Double-blind compliance was verified by re-extracting the PDF text and searching it for the author name, employer, prior employers, and the repo URL — all absent. Every form field was filled and read back via `playwright-cli` (title, short description 166/250 words, author + affiliation, presenter country, Primary Topic `Requirements`, Secondary Topic `Regression/Integration`, all three consent checkboxes, PDF attached). **Submit was deliberately NOT clicked** — the author is still refining the abstract. Two live-form facts were folded back into the skill's reference files: the Choose File `label.fu-hover` click target, and the Title counter being word-based rather than character-based.
- the DVCon U.S. 2027 extended abstract was revised after the 100-page wiki and submission skill were completed. Its new title, **"An AI Interface to 17 Years of DVCon,"** presents the repository as an open-source library deliberately built for both humans and AI agents across a conference knowledge loop—read, analyze, synthesize, and prepare contributions—rather than primarily as a RAG chat application. The revised markdown is 1,175 body words, within the 600–1,200-word requirement. Reference [6] links to the public GitHub repository; this intentionally de-anonymizes the abstract and was retained at the author's explicit direction despite the extended-abstract stage's double-blind rule.
...
- the `dvcon-submit` markdown converter now renders **inline formatting** instead of typing paragraph text verbatim. Previously `**bold**` and `*italic*` reached the PDF as literal asterisks. `convert_md_to_docx.ps1` gained `Split-InlineMarkdown` / `Get-InlineRuns`, which split each paragraph into character runs for `**bold**`/`__bold__`, `*italic*`/`_italic_`, `` `code` `` (Consolas, backticks dropped), and `[label](url)` (URL appended only when the label does not already contain it), honoring `\* \_ \` \[ \] \\` escapes and nesting bold/italic up to 4 levels. Fenced code blocks stay verbatim. Runs are separated by `Font.Reset()` and bold/italic are only ever turned **on** — never forced off — because `IEEE Abstract` is itself a bold style. The `_italic_` form requires non-word boundaries so DOIs and identifiers survive. Verified by re-extracting the regenerated `submissions/dvcon-us-2027/abstract.pdf`: zero leftover `**` / `](` markers, 2 `TimesNewRomanPS-BoldMT` spans and 7 `TimesNewRomanPS-ItalicMT` spans present, 25pt IEEE Title and small-caps ABSTRACT heading unchanged, still 3 pages US Letter (612x792).
- the bash twin of that converter now uses the **same parser** on macOS/Linux instead of a plain pandoc dump. `convert_md_to_docx.sh` still delegates to the `.ps1` on Windows. Elsewhere it converts the bundled `.doc` to `.docx` with LibreOffice, then `scripts/fill_ieee_docx.py` (Python 3 stdlib) rewrites `word/document.xml` with IEEE styleIds (`IEEETitle`, `IEEEText`, …), the Authors drop, and the same inline-run rules (`w:b`/`w:i`/Consolas only turned on, never forced off). Parser unit tests covered `***both***`, author-section drop, DOI underscores, and `[label](url)` expansion; filling a style-bearing template and exporting via Word produced bold/italic/bold-italic/Consolas runs with no leftover `**` and no leaked Authors text. pandoc is no longer a conversion dependency.
- the `dvcon-submit` skill now **fills the Accellera/DVCon copyright + speaker-consent PDF**. `scripts/fill_copyright_form.py` (PyMuPDF) writes the 2027 AcroForm (`references/dvcon-copyright-form-2027.pdf`): company name, title, all authors (stored in the misnamed `Title of Document 2` widget), presenter, presentation title, dates, optional paper-id suffix (the 2027 PDF has no Paper ID field), optional authorized-signer overlay, and the speaker-consent checkbox only when requested. Signature widgets stay blank unless the user supplies a PNG/JPG; PART B (U.S. Government) is never auto-filled. `--self-test` fills a temp copy of the bundled template and asserts those field values.
